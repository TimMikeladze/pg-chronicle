...
export async function ensureTestBucket(bucketName: string): Promise<void> {
	...
	const accessKeyId = process.env.PG_CHRONICLE_S3_ACCESS_KEY_ID || 'root'
	const secretAccessKey =
		process.env.PG_CHRONICLE_S3_SECRET_ACCESS_KEY || 'password'
	const region = process.env.PG_CHRONICLE_S3_REGION || 'us-east-1'
...
export async function putTestS3Object(
	...
	const client = new S3Client({
		endpoint: process.env.PG_CHRONICLE_S3_ENDPOINT || 'http://localhost:9000',
