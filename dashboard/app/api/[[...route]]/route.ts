/**
 * The real pg-chronicle REST API, mounted at /api.
 *
 * The dashboard itself does NOT go through this route — it calls the same
 * handlers in-process (see lib/pg-chronicle-server.ts). This mount exists so the
 * deployment is a complete pg-chronicle server for everything else: cron
 ...
 *
 * Required env: PG_CHRONICLE_DATABASE_URL, PG_CHRONICLE_TABLES, PG_CHRONICLE_JWT_SECRET.
 */
