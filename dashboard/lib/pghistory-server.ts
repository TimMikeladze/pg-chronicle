...
import { GET as pgHistoryGET, POST as pgHistoryPOST } from 'pghistory/next'
...
/**
 * The dashboard talks to pghistory by invoking the very same route handlers it
...
function assertSymmetricAlg(): void {
	const alg = process.env.PGHISTORY_JWT_ALG?.trim().toUpperCase()
	if (alg && !alg.startsWith('HS')) {
		throw new Error(
			`The dashboard signs its own tokens and needs a symmetric algorithm, but PGHISTORY_JWT_ALG is "${alg}". ` +
...
async function mintToken(): Promise<string> {
	const secret = process.env.PGHISTORY_JWT_SECRET
	if (!secret) {
		throw new Error(
			'PGHISTORY_JWT_SECRET is required. The dashboard exposes revert, so the API refuses to start without it.',
		)
	}
	assertSymmetricAlg()

	const alg = process.env.PGHISTORY_JWT_ALG?.trim().toUpperCase() || 'HS256'
	const now = Math.floor(Date.now() / 1000)

	return (
		new SignJWT({})
			// pghistory logs `sub` as the actor on every revert — that audit line is
 ...
 */
const INTERNAL_ORIGIN = 'http://pghistory.internal'
...
async function call<T>(
	...
	// 204 has no body; every other pghistory response is JSON.
...
/**
 * The OpenAPI document. pghistory JWT-gates `/openapi` unless `publicOpenApi`
