 ...
 *
 * The dashboard mints its own pg-history token on every request, so reaching a
 * page IS full read access plus revert — a destructive, unattributable-to-a-
 * person operation. Nothing in pg-history itself can stop that: from the API's
 ...
 *
 * Model: one shared password (`PG_HISTORY_DASHBOARD_PASSWORD`) exchanged for a
...
export const SESSION_COOKIE = 'pg_history_dashboard_session'

/** Paths that must stay reachable without a session. */
export const PUBLIC_PATHS = ['/login', '/health']

/**
 * `/api/*` is intentionally NOT gated by the cookie. It is the real pg-history
...
export function dashboardPassword(): string | undefined {
	return process.env.PG_HISTORY_DASHBOARD_PASSWORD?.trim() || undefined
}
 ...
 * `PG_HISTORY_DASHBOARD_ALLOW_ANONYMOUS=true` is the explicit, auditable opt-out
 * for deployments already behind an access proxy.
 */
export function anonymousAccessAllowed(): boolean {
	if (process.env.PG_HISTORY_DASHBOARD_ALLOW_ANONYMOUS?.trim() === 'true') {
		return true
	}
...
function sessionTtlSeconds(): number {
	const raw = process.env.PG_HISTORY_DASHBOARD_SESSION_TTL_HOURS?.trim()
...
async function sessionKey(password: string): Promise<Uint8Array> {
	const digest = await crypto.subtle.digest(
		'SHA-256',
		new TextEncoder().encode(`pg-history:dashboard:session:${password}`),
	)
