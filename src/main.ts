#!/usr/bin/env node
/**
 * Server entrypoint — `bun run src/main.ts`, or the published `pg-chronicle` bin
 * under Node.
 *
...
;(async () => {
	...
	const poolMax = Number.parseInt(process.env.PG_CHRONICLE_POOL_MAX || '5', 10)
	...
	// statement_timeout defaults to 30s; raise PG_CHRONICLE_STATEMENT_TIMEOUT_MS
	...
	const port = Number.parseInt(
		process.env.PG_CHRONICLE_PORT || process.env.PORT || '3001',
		10,
	)
	...
	if (process.env.PG_CHRONICLE_S3_BUCKET) {
		...
		archiverConfig = {
			...
			retention: { default: boundedInt('PG_CHRONICLE_RETENTION_DAYS', 90, 1) },
			// gracePeriod may be 0 (no grace — purge once the S3 backup is confirmed).
			gracePeriod: boundedInt('PG_CHRONICLE_GRACE_PERIOD_DAYS', 7, 0),
			batchSize: boundedInt('PG_CHRONICLE_BATCH_SIZE', 10000, 1),
		}
	}
	...
	const { app, dispose } = await createServer({
		...
		allowUnauthenticated:
			process.env.PG_CHRONICLE_ALLOW_UNAUTHENTICATED === 'true',
	})
