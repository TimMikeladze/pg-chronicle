...
;(async () => {
	...
	const poolMax = Number.parseInt(process.env.PGHISTORY_POOL_MAX || '5', 10)
	...
	// statement_timeout defaults to 30s; raise PGHISTORY_STATEMENT_TIMEOUT_MS
	...
	const port = Number.parseInt(
		process.env.PGHISTORY_PORT || process.env.PORT || '3001',
		10,
	)
	...
	if (process.env.PGHISTORY_S3_BUCKET) {
		...
		archiverConfig = {
			...
			retention: { default: boundedInt('PGHISTORY_RETENTION_DAYS', 90, 1) },
			// gracePeriod may be 0 (no grace — purge once the S3 backup is confirmed).
			gracePeriod: boundedInt('PGHISTORY_GRACE_PERIOD_DAYS', 7, 0),
			batchSize: boundedInt('PGHISTORY_BATCH_SIZE', 10000, 1),
		}
	}
	...
	const { app, dispose } = await createServer({
		...
		allowUnauthenticated:
			process.env.PGHISTORY_ALLOW_UNAUTHENTICATED === 'true',
	})
